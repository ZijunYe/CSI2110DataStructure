...
public class WeightGraph {
  ...
  Dijkstra<String,String> p = new Dijkstra<String,String>(); 
  p.execute(sGraph,vSource,WEIGHT); 
  for(Vertex<String>v: sGraph.vertices()){
    if(v.element().length()>2){
      System.out.println(vSource.element() +" to " + v.element() + " = " + p.getDist(v) );
    }
  }
    return;
